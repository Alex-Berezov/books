 ...
 *
 * ⚠️ Проверяются **все обработчики файла, а не один метод**: одиночный `try/catch`,
 * вернувшийся в файл, синтаксически ничем не отличается от правильного кода,
 * и ни `tsc`, ни линт, ни спеки сервиса его не видят. Их было десять; с 15.09.2026
 * девять — `overview` снят вместе с безъязыким `GET /books/:slug/overview`
 * (`LEGACY-387`).
 *
 * 🔴 **Полнота перечня не сверяется ничем: `HANDLERS` — рукописный список.** Новый
 * обработчик, добавленный в контроллер и не дописанный сюда, останется непокрытым
 * молча. Это известный пробел, а не недосмотр читателя: сверять пришлось бы разбором
 * файла контроллера, как это делает `src/common/testing/controller-decorators.ts`.
 *
