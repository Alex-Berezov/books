 ...
 * на оба пути**, а не копия у каждого: они берут один и тот же `FOR UPDATE` по строке
 * версии и ждут друг друга на нём. Разъехавшись, бюджеты сделали бы дедлайн зависящим
 * от того, кто пришёл к замку первым, — проигравший отдал бы `P2028` и 500 вместо
 * ответа. Комментарий «значения те же, что у соседа» такой связи не держит (`LEGACY-310`),
 * поэтому её держит имя.
 *
 * Дефолт Prisma (5000/2000 мс) рассчитан на пару операторов, а здесь их до пяти: замок
 * строки, запись статуса со снимком лицензий, `finalizeBaselineOnPublish` (своё чтение
 * версии со всеми главами, запись слепка и событие закрытия окна) и строка журнала.
 * На книге в несколько сотен глав дефолт отдаёт `P2028` и 500 **после** того,
...
export class BookVersionService {
  ...
  async update(id: string, dto: UpdateBookVersionDto) {
    ...
    try {
      const updated = await this.clearanceLock.runInLockedClearance(id, async (tx) => {
        ...
        //
        // 🔴 `LEGACY-320`: замок группы прав берётся не у каждой версии (только при
        // профиле или ревью), поэтому строка запирается отдельно и до чтения — иначе
        // встречная смена слага, закоммиченная между чтением и записью, оставалась без
        // редиректа. `FOR NO KEY UPDATE` — как у книги (решение арбитра 25.09.2026):
        // FK-вставки глав и связей таксономии не ждут правки.
        await tx.$queryRaw`SELECT id FROM "BookVersion" WHERE id = ${id} FOR NO KEY UPDATE`;
  ...
  /**
   * `LEGACY-015`. Публикация пишет парное событие к `VERSION_UNPUBLISHED` **той же
   * транзакцией**, что и сам статус. Без него журнал не молчал, а врал: снятие
   * с публикации записано с 12.09.2026, а самой публикации в журнале не было —
   * по нему выходило, что версию сняли с публикации, которой не случалось.
   *
   * `actorUserId` приходит параметром, а не читается из тела запроса: «кто», а не
   * «кто представился» (тот же приём, что у `unpublish` и у смены ролей).
   */
  async publish(id: string, actorUserId: string | null) {
    ...
    const published = await this.prisma
      .$transaction(async (tx) => {
        ...
        // помощником и в том же порядке, что у парного `unpublish`. Нужен он ровно
        // для одного: событие пишется только тогда, когда состояние действительно
        // изменилось (инвариант модели `AdminAuditEvent`). Повторная публикация уже
        // опубликованной версии проходит как и раньше, но второй строки в журнал
        // не кладёт — иначе на вопрос «когда её опубликовали» журнал отдаёт столько
        // ответов, сколько раз нажали кнопку.
        //
        // Чтение без замка здесь не годится: под READ COMMITTED две встречные
        // публикации одной версии обе прочитали бы `draft` и обе записали бы событие.
        const before = await lockLicenseSnapshot(tx, id);
        // Версию удалили между чтением выше и замком: публиковать нечего.
        if (!before) throw new NotFoundException('BookVersion not found');
        ...
        // Пишется общим писателем и тем же `tx`, что и статус: событие, пережившее
        // откат своей транзакции, утверждает публикацию, которой не было (`LEGACY-036`).
        // В `payload` — тот же лицензионный снимок, что записан оператором выше, и той
        // же формой, какой его уносит снятие: иначе две половины одной истории версии
        // читаются двумя разными выборками. Ни почты, ни имени (инвариант модели).
        //
        // Условие — то же самое, что у выдачи роли: событие равно изменению состояния.
        // Состояний здесь два, и оба считаются изменением (решение арбитра 20.09.2026):
        // переход в опубликованное и смена самого покрытия. Второе важно потому, что
        // повторная публикация **физически** перезаписывает снимок: отозвали лицензию,
        // купили другую, нажали «Опубликовать» — прежние значения исчезают, и без события
        // на вопрос «на чём стояла выдача до сегодня» не ответит ни строка, ни журнал
        // (`ADR-009`). Обе временные метки в сравнение не входят: они пишутся
        // `new Date()` на каждый вызов, и условие выродилось бы в «писать всегда».
        if (!wasPublished || licenseSnapshotChanged(before, updated)) {
          await this.adminAudit.record(tx, {
            action: AdminAuditAction.VERSION_PUBLISHED,
            targetType: AdminAuditTargetType.BOOK_VERSION,
            targetId: id,
            actorUserId,
            payload: licenseSnapshotPayload(updated),
          });
        }

        return updated;
      }, BOOK_VERSION_VISIBILITY_TX_OPTIONS)
      .catch((e: unknown) => {
  ...
  async unpublish(id: string, actorUserId: string | null): Promise<BookVersionWithSeo> {
    ...
    const updated = await this.prisma
      .$transaction(
        ...
        // её до конца пула соединений, если вызывающий повис (`L-019`). Константа общая
        // с публикацией: оба пути стоят в очереди к одному замку.
        BOOK_VERSION_VISIBILITY_TX_OPTIONS,
      )
