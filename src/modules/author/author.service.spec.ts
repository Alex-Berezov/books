...
describe('AuthorService', () => {
  ...
  describe('update', () => {
    ...
    /**
     * `LEGACY-085`, утверждающий тест — третья сущность класса, рядом с категорией
     * и тегом.
     *
     * Язык, выпавший из `dto.translations`, теряет адрес: его перевод сносит общий
     * `deleteMany`, а `record()` по нему не зовётся — цикл идёт по `dto.translations`,
     * где этого языка уже нет. Переименование при этом редирект пишет: два языка
     * в одном вызове ведут себя по-разному, и тест проверяет обе половины сразу,
     * иначе «не позвали» ничего не доказывает.
     *
     * Владелец 15.09.2026 выбрал вариант D: редирект на родителя там, где родитель
     * есть, и 404 там, где его нет. У `Author` родителя нет **по построению схемы**
     * (`prisma/schema.prisma`, `model Author` — есть только `personId`, это другая
     * сущность), поэтому 404 здесь — исполненное решение, а не отложенная починка.
     * Красное означает, что редирект автора завели в обход решения.
     */
