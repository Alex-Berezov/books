...
export class ImportService {
    ...
    for (const item of checked) {
      ...
      try {
        ...
        if (prismaErr.code === 'P2002') {
          ...
          // (перевод снёс `DELETE /tags/:id/translations/:language`). Обе ручки
          // берут замок строки тега — `remove` с 15.09.2026 (`LEGACY-395`),
          // ручки переводов с 16.09.2026 (`LEGACY-360`), — и под ним ветка должна
          // быть недостижима; она оставлена на случай писателя мимо `TagLockService`.
          // Назвать одну причину значило бы отправить оператора проверять тег,
          // который на месте.
          result.errors.push({
            key: item.key,
            message:
  ...
  private async upsertCategory(
    ...
    //
    // 🔴 `LEGACY-276`. Вторым оператором идёт замок по слагу — тем же входом
    // и в том же порядке «дерево → слаг», что у админских путей; разойтись им
    // негде, порядок держит конструкция.
    //
    // ⚠️ Берётся он на каждый термин партии, **включая ветку обновления**,
    // и это стоит очереди: слаг существующему термину импорт не переписывает
    // (см. комментарий у `data` ниже), то есть замок берётся по адресу, который
    // эта транзакция может и не писать, — а чужой `POST` с тем же слагом ждёт.
    //
    // Платится это сознательно: развилку «создать или обновить» нельзя решать
    // до замка. Унести `lockCategorySlug` под `if (!existing)` значит вернуть
    ...
    return this.categoryTree.runInLockedTree(async (tx) => {
      ...
      if (!existing) {
        ...
        // гонка: `commonData.slug` выводится `getFirstSlug`-ом из первого
        // перевода по порядку ключей JSON, и два разных `key` с одинаковым
        // первым слагом создавали оба термина отчётом `imported`, без единой
        // проверки — правило занятости слага сюда не заглядывало вовсе.
        //
        // Правило одно на оба пути записи и живёт в `CategoryTreeService`
        // (`LEGACY-311`, `LEGACY-131`): своя копия здесь разошлась с админской
        // в день заведения — другой текст отказа и отсутствующий `exceptId`.
        // `exceptId` тут и не нужен: строки ещё нет, исключать из проверки
        // нечего.
        //
   ...
   *
   * 1. Два одновременных создателя одного ключа (импорт, `POST /tags`) с 16.09.2026
   *    сериализует **advisory-замок ключа** (`TagLockService.lockTagKey`,
   *    `LEGACY-320`): второй ждёт коммита первого, видит строку и идёт веткой
   *    обновления. Уникальный индекс `Tag_key_key` остаётся страховкой:
   *    `P2002` и `Duplicate key` по `key` возможны только при писателе мимо
   *    замка; по слагу — по-прежнему. Общей на все теги блокировки, как
   *    у дерева категорий (`LEGACY-274`), нет намеренно (решение арбитра
   *    от 29.08.2026): замок берётся на один ключ.
   ...
   *
   *    С 16.09.2026 развилку по `existing.translations` держат все её писатели:
   *    админские ручки переводов (`POST/PATCH/DELETE /tags/:id/translations`)
   *    берут тот же замок строки (`LEGACY-360`). Путь по `key` перед пробой
   *    строки берёт advisory-замок ключа, и его же берёт `POST /tags`: сосед,
   *    создающий строку, ждёт коммита, а не появляется между пробой
   *    и `findUnique` ниже.
   *
  ...
  private async upsertTag(dto: ImportTagDto): Promise<{ created: boolean }> {
    ...
    //
    // Развилку «создать или обновить перевод» ниже держат и ручки переводов:
    // они берут тот же замок (блок 3 в шапке метода, `LEGACY-360`).
    //
