 ...
 * slug already exists», — и он был неверен всё то время, пока индекса
 * на `Category.slug` не было: с `20250830151000_add_taxonomy_translations`
 * `@unique` стоял только на `key`, то есть `P2002` приходил сюда **по ключу**,
 * а оператор читал про слаг — менял слаг и получал тот же 400 сколько угодно раз.
 *
...
export class CategoryService {
  ...
  async create(dto: CreateCategoryDto) {
    ...
    // Термин без родителя ребра не пишет: очередь **дерева** здесь брать
    // не за чем, и брать её значило бы сериализовать массовое заведение
    // корневых терминов на общей очереди импорта (`LEGACY-274`).
    //
    // 🔴 `LEGACY-276`. Замок по **слагу** здесь всё равно берётся: на
    // READ COMMITTED две одновременные вставки не видят незакоммиченных строк
    // друг друга, поэтому без замка два одновременных `POST` с одним слагом
    // и разными ключами проходили бы оба. С `@@unique([slug])` (релиз 2,
    // миграция `20260919170000_legacy_276_category_slug_unique`) второй коммит
    // отбила бы уже база, и 400 клиент всё равно получил бы — `writeCategoryRow`
    // превращает `P2002` в `BadRequest`. Но узнавал бы проигравший об этом
    // только после попытки записи, а в логе на штатной ошибке оператора лежал
    // бы отказ базы. Порядок держит вход `runInTree`, а не комментарий здесь
    // (`LEGACY-310`); решение арбитра 19.09.2026.
    if (!dto.parentId) {
      return this.categoryTree.runInTree(async (tx) => {
        await this.categoryTree.assertSlugFree(tx, dto.slug);
        return this.writeCategoryRow(() => tx.category.create({ data }));
      }, dto.slug);
    }
  ...
  /**
   * Запись строки **самого термина** и только её: `P2002` по ключу или
   * по слагу таксономии — это занятый адрес, то есть 400, а не 500.
   *
   * ⚠️ Вынесено отдельно потому, что запись идёт из трёх мест — `create`
   * с блокировкой и без неё, `update` — а три копии `catch` разошлись бы
  ...
  async update(id: string, dto: UpdateCategoryDto) {
    ...
    // 🔴 `LEGACY-276`. Проверок занятости слага **две**, и это не дубль.
    //
    // Эта — дешёвый отказ на пуле, до открытия транзакции: обычная ошибка
    // оператора («завожу термин на занятый адрес») обязана стоить 400 сразу
    // и не занимать собой очередь. Перенос её внутрь `body` заходом 19.09.2026
    // был регрессом: `touchesTree` срабатывает почти всегда — админка шлёт
    // сущность целиком, — и заведомо отвергаемый `PATCH` вставал за `lockTree`,
    // удерживая соединение единственного пула, а на партии импорта длиннее
    // потолка отдавал `P2028` и 500 вместо 400 (`LEGACY-256`).
    //
    // Настоящий рубеж — не она: её снимок к моменту записи устаревает. Пару
    // «проверил — записал» сериализует замок по слагу внутри транзакции,
    // а уникальность в базе заводит `@@unique([slug])` — заведена релизом 2
    // (миграция `20260919170000_legacy_276_category_slug_unique`).
    // Решение арбитра 19.09.2026, строка в `decisions-log.md`.
    if (dto.slug) {
      await this.categoryTree.assertSlugFree(this.prisma, dto.slug, id);
    }
    ...
    const body = async (tx: Prisma.TransactionClient) => {
      ...
      // оператором: сосед с тем же слагом ждёт коммита, а не вклинивается
      // между этой проверкой и записью. `exceptId` — сам термин, чтобы PATCH,
      // не меняющий слаг, не отвергал сам себя.
      if (dto.slug) {
        await this.categoryTree.assertSlugFree(tx, dto.slug, id);
      }
      ...
      // `LEGACY-276`: до сих пор эта запись не ловила `P2002` вовсе — гонка на
      // слаге, прошедшая мимо `assertSlugFree` этой же транзакции, отвечала
      // бы 500, а не 400. `create` через `writeCategoryRow` ловит с 29.08.2026
      // (`LEGACY-311`), у `update` копии не было.
      return this.writeCategoryRow(() =>
        tx.category.update({
          where: { id },
          data: {
            type: dto.type,
    ...
    // 🔴 `LEGACY-276`. Замок по слагу берётся по условию «`dto.slug` пришёл»,
    // а не безусловно: ключ замка — слаг из тела запроса, и узнать его иначе
    // можно только чтением строки, а чтение до замка лишает замок смысла
    // (та же причина, что у `touchesTree` выше). `PATCH`, слага не меняющий,
    // писателем слага не является и в эту очередь не встаёт.
    return touchesTree
      ? this.categoryTree.runInLockedTree(body, dto.slug)
      : this.categoryTree.runInTree(body, dto.slug);
  }
   ...
   *
   * 🔴 `LEGACY-392`. Адресов здесь умирает **два вида**, и второй долго оставался
   * без редиректа вовсе. Кроме слагов переводов умирает **базовый** `Category.slug`:
   * резолв ищет пару `language_slug`, а не найдя — падает на него и отдаёт страницу
   * запрошенного языка. С 22.09.2026 с него пишется 308 на перевод родителя того же
   * языка, **одним переходом**, а не через базовый слаг родителя.
   *
   * 🔴 **Языки этого 308 — те, где у категории был перевод, а не все мёртвые.**
   * Живым адресом базовый слаг был только там: без перевода фоллбэк отдаёт 200
   * с `translation: null`, фронт уходит в `notFound()` до истории слагов, и адрес
   * отвечал 404 ещё **до** удаления — 308 там заводил бы новый публичный адрес
   * вместо возврата умершего. Поэтому перебор идёт по умершим переводам, а `deadLanguages`
   * служит здесь только отсевом языков, где слаг остался жив. Уборка ниже — наоборот,
   * по всем мёртвым языкам: она снимает строки, ведущие **НА** слаг, а их писал
   * `recordBaseSlugChange` сразу на все пять. Множества разные намеренно, сводить
   * их нельзя (три решения арбитра 22.09.2026, `decisions-log.md`).
   *
   * Язык, где слаг умершего перевода совпадал с базовым, пропускается — адрес там
   * один, а не два.
   */
  async remove(id: string, actorUserId: string | null) {
    return this.categoryTree.runInLockedTree(async (tx) => {
      ...
      // Базовый слаг тоже умер. Живость адреса решается **в каждом языке отдельно**
      // (`LEGACY-394`), поэтому дальше идёт список мёртвых языков, а не один слаг
      // на всю историю: в языке, где тот же слаг держит чужой живой перевод,
      // публичный резолв отвечает 200 по паре `language_slug` раньше, чем успевает
      // дойти до фоллбэка на `Category.slug`.
      const deadLanguages = await this.deadLanguagesForSlug(tx, removed.slug);

      // Второй умерший адрес — базовый `Category.slug` (`LEGACY-392`). Политика та же,
      // что у слагов переводов, и живёт она в `recordAddressSuccessor`; здесь — только
      // множество языков, и оно своё.
      //
      // 🔴 **Перебор по `dying`, а не по `deadLanguages`.** Живым адресом базовый слаг
      // был только там, где у категории был перевод: без перевода фоллбэк отдаёт 200
      // с `translation: null`, а фронт уходит в `notFound()` **до** истории слагов
      // (`books-front/app/[lang]/category/[slug]/page.tsx`). Значит там адрес отвечал
      // 404 и **до** удаления, и 308 на нём заводил бы новый публичный адрес вместо
      // возврата умершего (решение арбитра 22.09.2026 по находке ревью).
      //
      // 🔴 **Множество записи уже множества уборки намеренно — сводить их нельзя.**
      // Уборка ниже снимает строки, ведущие **НА** мёртвый слаг: их писал
      // `recordBaseSlugChange` сразу на все языки, независимо от переводов. Этот цикл
      // выдаёт 308 **С** него. Инвариант `LEGACY-394` про первое.
      //
      // 🔴 Запись идёт **до** уборки: `record` переписывает цепочки, и уборка первой
      // сняла бы их раньше, чем они успели бы переехать (`LEGACY-390`).
      const deadLanguageSet = new Set(deadLanguages);

      for (const tr of dying) {
        // Слаг перевода совпал с базовым — адрес один, а не два, и цикл выше его уже
        // обслужил. Так термины заводит сид, поэтому случай рядовой; без пропуска
        // `record` звался бы второй раз с теми же аргументами и стоил три лишних
        // обмена под глобальным замком дерева (находка ревью 22.09.2026).
        if (tr.slug === removed.slug) continue;
        // Слаг остался живым — его держит чужой перевод. 308 увёл бы посетителя
        // с работающей страницы.
        if (!deadLanguageSet.has(tr.language)) continue;
        await this.recordAddressSuccessor(tx, {
          language: tr.language,
          dyingSlug: removed.slug,
          parentSlug: parentSlugByLanguage.get(tr.language),
        });
      }

      // Записи, которые вели НА базовый слаг, теперь вели бы в 404. Те из них, что
      // указывали на язык с живым родителем, цикл выше уже перевёл на преемника;
      // здесь снимается остаток.
      //
      // 🔴 Множество здесь шире, чем у записи выше, и это разобрано там же:
      // `recordBaseSlugChange` писал строки на этот слаг сразу на все языки,
      // поэтому снимать их надо по всем мёртвым — иначе строка на язык,
      // не перебранный уборкой, останется 308-м в 404.
      //
  ...
  /**
   * Политика ухода публичного адреса **перевода** термина (`LEGACY-390`, решение
   * арбитра 15.09.2026).
   *
   * 🔴 **Адресов смерти три, а этот метод обслуживает два из них.** Путь перевода —
   * `deleteTranslation`, путь категории целиком — `remove()`, и там этот метод
   * зовётся в цикле по умершим переводам. Третий адрес — **базовый** `Category.slug`,
   * он умирает только в `remove()` и обслуживается там отдельным циклом. Различий
   * два, и оба намеренные: 308 с базового слага пишется по языкам **умерших
   * переводов** (только там он и был живым адресом), а уборка там идёт одним
   * батчевым запросом по всем мёртвым языкам, а не по языку. Признак живости
   * тоже свой — `deadLanguagesForSlug` вместо поязыкового `isBaseSlugTaken`;
   * сводить их в один нельзя, они разные по существу (`excludeCategoryId` ниже
   * про то же). Разбор — в докблоке `remove()`.
   *
   * Общее у всех трёх адресов ровно одно — **выбор преемника**, и он вынесен
   * в `recordAddressSuccessor` (решение арбитра 22.09.2026); какие правки политики
   * ложатся туда и почему — в докблоке самого метода, здесь копии нет намеренно.
   *
   * Пока политика жила копией в каждом пути, копии успели разойтись за один
   * заход — ровно тот разъезд, ради запрета которого запись и заведена.
   *
  ...
  private async retireCategoryAddress(
    ...
    await this.recordAddressSuccessor(tx, { language, dyingSlug, parentSlug });
  ...
  /**
   * Выбор преемника умершего публичного адреса — **одно место на все три адреса**
   * (решение арбитра 22.09.2026).
   *
   * Адресов три: слаг перевода на двух путях его смерти (`deleteTranslation`
   * и цикл по `dying` в `remove()`) и базовый `Category.slug` в `remove()`.
   * Признак живости и форма уборки у них разные и разведены намеренно
   * (докблок `retireCategoryAddress`), а вот преемник выбирается одинаково —
   * и до выноса эти три строки стояли копией в каждом цикле.
   *
   * Политика D1 (решение владельца 15.09.2026): преемник — перевод **прямого**
   * родителя на **том же** языке. Нет его — редиректа не пишется вовсе, адрес
   * остаётся 404. Достраивать адресата за владельца нельзя: 308 на несуществующий
   * адрес хуже 404, потому что невыданный 308 чинится вторым заходом, а выданный
   * из поискового индекса не отзывается.
   *
   * 🔴 Ожидаемые правки этой политики — «нет перевода у родителя, идём к деду»,
   * «не писать 308 на скрытого родителя», «не писать 308 на неиндексируемый
   * перевод» — ложатся сюда и действуют сразу на все три адреса. Ни уборки,
   * ни проверки живости здесь быть не должно: они у каждого пути свои.
   */
  private async recordAddressSuccessor(
    tx: Prisma.TransactionClient,
    params: { language: Language; dyingSlug: string; parentSlug: string | undefined },
  ): Promise<void> {
    const { language, dyingSlug, parentSlug } = params;
   ...
   * `language_slug`, и лишь при промахе — фоллбэк на базовый `Category.slug`.
   *
   ...
   * уборка и написана.
   *
   * 🔴 **Ранний выход по `isBaseSlugTaken(tx, slug, undefined)` остаётся и после
   * `@@unique([slug])`** (`LEGACY-276`, релиз 2; разбор ревью 19.09.2026). Соблазн
   * снять его как недостижимый выглядит обоснованным: `slug` здесь — базовый слаг
   * строки, которую `remove()` уже удалил строкой выше, а уникальное ограничение
   * не даёт двум живым строкам держать одно значение, поэтому при живом индексе
   * запрос всегда отвечает «свободен».
   *
   * Условие, при котором он всё-таки отвечает «занят», — **дубли в базе без индекса**,
   * и это не гипотеза: `scripts/docker-entrypoint.sh` зовёт `prisma migrate deploy`
   * и глотает его отказ (`|| echo …`), а `ADR-018`, раздел «Цена», прямо требует
   * эту строку не трогать — на проглатывании держится откат. Миграция падает ровно
   * на дублях (`23505`), то есть новый образ штатно поднимается против базы без
   * индекса именно тогда, когда дубли есть. Снятый выход в этом состоянии стирает
   * `slugRedirect` по всем пяти языкам для слага, который ещё отвечает 200 через
   * фоллбэк по второй строке: выданный 308 превращается в 404, а он из поискового
   * индекса не отзывается. Цена сохранения — один `findFirst` по уникальной колонке
   * в транзакции, где и так семь с лишним запросов.
   *
   * ⚠️ У близнеца — `TagsService.deadLanguagesForTagSlug` — тот же выход нужен ещё
   * и без всяких оговорок: у `Tag.slug` уникальности нет вовсе (`schema.prisma`,
   * модель `Tag`: `@unique` только на `key`), два живых тега с одним базовым слагом
   * там законны.
   *
  ...
  private async deadLanguagesForSlug(
    ...
    //
    // 🔴 С `@@unique([slug])` (релиз 2 `LEGACY-276`) этот выход почти всегда ложен —
    // но именно «почти»: он и есть защита на состояние «дубли есть, миграция упала,
    // образ поднялся», которое энтрипойнт делает штатным. Разбор — в докблоке выше.
   ...
   *
   * ⚠️ **Чья категория считается — решает `excludeCategoryId`, а не этот заголовок.**
   * Без него в отбор входит и своя строка, и это не небрежность: на пути
   * `deleteTranslation` категория остаётся жить, и её собственный базовый слаг
   * продолжает отвечать 200. Исключение своей строки там превратило бы живой адрес
   * в 308 с уборкой записей, ведущих на работающую страницу (класс `LEGACY-085`).
   *
   ...
   *
   * 🔴 `LEGACY-276`, релиз 2: вызов из `deadLanguagesForSlug` **сохранён** и после
   * появления `@@unique([slug])`. Почему недостижимость здесь кажущаяся — в докблоке
   * того метода: энтрипойнт глотает отказ `migrate deploy`, а падает он ровно
   * на дублях.
   */
