 ...
 * (`uniqueViolationMessage` в `category.service.ts`). С релизом 2, заведшим
 * `@@unique([slug])` (миграция `20260919170000_legacy_276_category_slug_unique`),
 * обе достижимы на одном и том же запросе — кто первым поймает, тот и ответит, —
 * и разные тексты на одно событие были бы видны оператору как две разные ошибки.
 */
...
export class CategoryTreeService {
   ...
   * `decisions-log.md`).
   *
   * Замок написан до `@@unique([slug])` (индекс `Category_slug_key`, снесённый
   * миграцией `20250830151000_add_taxonomy_translations`, возвращён миграцией
   * `20260919170000_legacy_276_category_slug_unique`, релиз 2 этой записи) —
   * и остаётся нужен после её появления в базе: без него `assertSlugFree` ловит
   * только тот дубль, который уже закоммичен, — на `read committed` два
   * одновременных писателя одного слага не видят незакоммиченных строк друг
   * друга и проходят проверку оба. Замок закрывает именно это окно — второй
   * ждёт коммита первого и проверяет уже по его строке, поэтому до самой базы
   * такая гонка не доходит вовсе и `P2002` остаётся аварийным рубежом, а не
   * штатным ответом.
   *
   ...
   * их сюда же.
   *
   * ⚠️ Сама по себе эта проверка рубежом не была и до `@@unique([slug])`
   * (релиз 2, миграция `20260919170000_legacy_276_category_slug_unique`,
   * `ADR-018` класс 2) — пару «проверил — записал» сериализовал только
   * `lockCategorySlug` у всех писателей слага. С индексом в базе рубеж есть,
   * но проверка не становится лишней: она превращает ожидаемый дубль в 400
   * до обращения к базе, а не полагается на то, что `P2002` долетит и будет
   * поймано на каждом из путей записи.
   *
