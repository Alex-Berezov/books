...
describe('SeoService (unit)', () => {
  ...
  describe('resolvePublic — отказ базы на необязательном блоке не пропадает молча (LEGACY-277)', () => {
    ...
    describe('страница книги', () => {
      ...
      /**
       * 🔴 `LEGACY-005`. Список жанров читал связь через `include`, то есть выбирал бы
       * все скаляры `BookCategory`, включая мёртвую `isPrimary`. Из связи нужен
       * только перевод термина.
       */
      it('жанры читаются белым списком, а не include на связи', async () => {
        await resolveBook();
