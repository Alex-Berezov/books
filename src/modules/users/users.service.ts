 ...
 * их больше: чтение прежнего набора, замена, запись событий — по одному оператору на каждое
 * событие журнала (`LEGACY-015` пункт 6, `AdminAuditService.record` вызывается последовательно,
 * не `createMany`). У `deleteById` цепочка ещё длиннее — около дюжины операторов, от лайков
 * и комментариев до записи событий на каждую снятую роль плюс `USER_DELETED`, — и именно она
 * упирается в дефолт первой. Ужимая эти значения, помни про оба потребителя.
 *
...
export class UsersService {
  ...
  async deleteById(userId: string, actorUserId: string | null): Promise<PublicUser> {
    ...
    const deleted = await this.prisma.$transaction(
      ...
      // добавила к цепочке оператор на каждую снятую роль плюс `USER_DELETED`, а дефолт
      // Prisma (5000/2000) рассчитан на пару. Значения те же, что у смены ролей выше.
  ...
  /**
   * Ролевой писатель журнала административных действий (`LEGACY-015`), переехавший
   * на общий `AdminAuditService` (`LEGACY-015`, пункт 6, 26.09.2026). Не единственное
   * место записи: события пишет `this.adminAudit.record(tx, ...)`, а этот метод только
   * раскладывает разницу наборов ролей на отдельные вызовы той же транзакцией.
   *
   ...
   * не делает ни одного вызова. Инвариант «событие = изменение состояния» описан
   * в doc-комментарии модели `AdminAuditEvent`.
   *
   * ⚠️ Вызовы `record()` идут **последовательно**, не `Promise.all`: тот же клиент
   * транзакции у Prisma не терпит параллельных запросов на одном соединении.
   */
  private async recordRoleAuditEvents(
    ...
    for (const { action, role } of events) {
      await this.adminAudit.record(tx, {
        action,
        targetType: AdminAuditTargetType.USER,
