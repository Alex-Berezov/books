...
describe('PersonsService.remove — проверка связей падает закрыто', () => {
   ...
   *
   * ⚠️ Текст SQL сверяется отдельным кейсом ниже. Про него важно не преувеличить: имя таблицы
   * внутри тега `$queryRaw` **проверяет `yarn drift-check`** — он разбирает сырые шаблоны
   * и краснеет на идентификаторе, которого нет в схеме (проверено подстановкой `"Persons"`:
   * `RAW SQL IDENTIFIERS ABSENT FROM SCHEMA`). Чего не проверяет никто, кроме кейса ниже, —
   * это само `FOR UPDATE`: снятое, оно оставляет запрос синтаксически исправным и ссылающимся
   * на живую таблицу, а замок превращает в обычное чтение.
   *
   * Живой гонки на настоящей базе не проверяет и кейс ниже: e2e на этот путь
   * (`DELETE /admin/contributors/:id`) в репозитории нет ни до правки, ни после. Записью
   * не оформлено: пробел в покрытии порога заведения не проходит (`tech-debt-autopilot.md`,
   * «Порог заведения записи»).
   */
