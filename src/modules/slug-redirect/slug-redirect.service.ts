...
export class SlugRedirectService {
  ...
  /**
   * Смена **базового** слага сущности — того, что не привязан к языку.
   *
   * Публичный URL резолвится по слагу перевода **с фолбэком на базовый**, поэтому
   * старый базовый слаг ломается сразу во всех языках. Запись поэтому идёт для
   * каждого языка: `SlugRedirect` привязан к языку, и «язык не указан» здесь
   * означало бы редирект, не работающий нигде.
   *
   * Назначение — новый **базовый** слаг, а не слаг перевода. Посетитель попадёт на
   * него, страница найдёт термин и уже существующий `resolveTaxonomyDestination`
   * уведёт на локализованный адрес. Два перехода вместо одного — плата за то, что
   * здесь мы не знаем переводов; зато правило остаётся одно и не зависит от того,
   * заведён ли перевод на конкретный язык.
   */
  async recordBaseSlugChange(
    entityType: SlugRedirectEntityType,
    oldSlug: string,
    newSlug: string,
    tx?: PrismaLike,
    ...
    for (const language of Object.values(Language)) {
      await this.record({ entityType, language, oldSlug, newSlug }, tx);
    }
  }

  /**
   * Снять записи, ведущие НА адрес, который только что умер (`LEGACY-395`).
   *
   * Писатель истории (`record`/`recordBaseSlugChange`) обязан быть и её уборщиком:
   * сущность удалена — старые 308 на её бывший слаг обязаны либо уйти на живого
   * преемника (это решает вызывающий, здесь такого шага нет), либо, если преемника
   * нет, сняться сами — иначе они ведут в 404, а выданный 404 из поискового индекса
   * не отзывается.
   *
   * Уборка всегда **по языкам** (`deadLanguages`), а не по слагу вообще: живость
   * публичного адреса — вопрос про язык (`LEGACY-394`), и уборка обязана перебирать
   * то же множество, по которому шла запись. Само по себе множество и решение,
   * жив ли адрес в каждом языке, — забота вызывающего: оно разное для каждой формы
   * слага (базовый + переводы у категории и тега, один слаг без языка у книги,
   * слаг+язык без базового у страницы и версии книги) и здесь не обобщается.
   */
  async cleanupDeadRedirects(
    entityType: SlugRedirectEntityType,
    deadLanguages: Language[],
    deadSlug: string,
    tx?: PrismaLike,
  ): Promise<void> {
    ...
    await db.slugRedirect.deleteMany({
      where: { entityType, language: { in: deadLanguages }, newSlug: deadSlug },
    });
  }
