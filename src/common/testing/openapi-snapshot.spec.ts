 ...
 * открывает `IORedis`; `media-jobs.module.ts` поднимает воркер `media-probe`,
 * а `MediaCleanupSchedulerService` (с 21.09.2026, `LEGACY-059`) заводит таймер,
 * который зовёт `cleanup()` и удаляет строки `MediaAsset` и файлы в хранилище
 * по адресам из окружения. Preview-режим не создаёт ни одного из них, а Swagger
 * читает метаданные контроллеров, а не их экземпляры.
 *
