 ...
 *
 * Уникальности на `Category.slug` в базе нет: индекс `Category_slug_key` снесён миграцией
 * `20250830151000_add_taxonomy_translations`, и вернёт его только релиз 2 записи. До тех пор
 * пару «проверил слаг — записал» держит **одно** — advisory-замок по хешу слага, который
 * берут входы `CategoryTreeService.runInTree` и `runInLockedTree` необязательным вторым
 * аргументом.
 *
