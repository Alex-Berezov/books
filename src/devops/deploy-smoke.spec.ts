...
/**
 * Диагностический скрипт читается **лениво**, а не на верхнем уровне модуля:
 * сведение двух списков к одному источнику (второй путь, разрешённый рекомендацией
 * `LEGACY-233`) может означать удаление самого файла, и тогда `readFileSync` при
 * загрузке модуля уронил бы ENOENT всю спеку - вместе с пятнадцатью кейсами дыма
 * выката, к скрипту отношения не имеющими. Так падение остаётся в своих трёх кейсах.
 */
const checkBibliaris = (): string =>
  readFileSync(join(ROOT, 'scripts', 'check_bibliaris.sh'), 'utf8');
...
describe('DevOps: дым по маршрутам на выкате', () => {
  ...
  /**
   * LEGACY-233. `scripts/check_bibliaris.sh` держит второй, независимый список
   * маршрутов и повторяет тот же antipattern, что чинился здесь по `LEGACY-232`:
   * `curl -w '%{http_code}' ... || echo "000"`. curl на отказе соединения печатает
   * `000` САМ (проверено этой же сборкой строкой ниже), поэтому `|| echo "000"`
   * не восстанавливает поведение при отказе - он дописывает вторую "000" к уже
   * напечатанной, и переменная становится строкой "000000", не совпадающей ни
   * с одним ожиданием кода. Голое удаление `|| echo "000"` без `set +e`/`set -e`
   * сломало бы скрипт иначе: `set -euo pipefail` в шапке файла останавливает
   * скрипт на первом же отказе curl внутри `x=$(curl ...)` - без диагностики
   * и без остальных проверок.
   */
  describe('LEGACY-233: check_bibliaris.sh не глотает код отказа curl', () => {
    /**
     * Строки вызова curl, несущие write-out (`-w "%{...}"`), без комментариев.
     *
     * 🔴 Один предикат на оба кейса ниже, и он про **форму вызова**, а не про
     * литерал фолбэка. Литеральный поиск `%{http_code}` + `echo "000"` (первая
     * редакция этой спеки) пробивается четырьмя способами сразу: `|| echo 000`
     * без кавычек, `|| echo '000'` в одинарных, другой текст фолбэка и — главное —
     * другой write-out: `-w "%{time_total}s" ... || echo "timeout"` в
     * `check_performance` был ровно тем же дефектом и под фильтр не попадал.
     *
     * Комментарии срезаны по той же причине, что и в `verifyLoopLines()` выше:
     * шапка самой правки объясняет дефект и потому содержит и `%{http_code}`,
     * и `|| echo` — без среза кейс краснел бы на исправном скрипте от одного лишь
     * переноса строки в тексте пояснения.
     */
    const writeOutCalls = (): { line: string; i: number }[] =>
      checkBibliaris()
        .split(/\r?\n/)
        .map((line, i) => ({ line, i }))
        .filter(({ line }) => /-w\s+["']?%\{/.test(line) && !line.trim().startsWith('#'));

    it('ни один вызов curl с write-out не глушит отказ хвостом `|| echo`', () => {
      const calls = writeOutCalls();
      // Ниже пяти сломан разбор, а не скрипт поредел: четыре `%{http_code}`
      // (check_http, check_https дважды, check_endpoints) и один `%{time_total}`
      // (check_performance). Приём из `route-order.spec.ts`.
    ...
    // Без этого кейса голое снятие `|| echo` до первого прогона выглядело бы
    // рабочим: `set -e` молчаливо останавливает скрипт на отказавшем curl, и все
    // проверки после него просто не печатаются - симптом незаметен на успешном
    // прогоне и обнаруживается только на отказавшем хосте.
    it('каждый вызов curl с write-out обёрнут set +e / set -e', () => {
      const lines = checkBibliaris().split(/\r?\n/);
      const calls = writeOutCalls();
      expect(calls.length).toBeGreaterThanOrEqual(5);

      for (const { i } of calls) {
        const before = lines.slice(Math.max(0, i - 3), i).map((l) => l.trim());
        const after = lines.slice(i + 1, i + 4).map((l) => l.trim());
        expect(before).toContain('set +e');
        expect(after).toContain('set -e');
      }
    });

    // Дым деплоя (REQUIRED выше) и список check_bibliaris.sh - два независимых
    // источника одного класса маршрутов (LEGACY-233, п.1: «копии разъедутся
    // молча, расхождение конвейеров этот проект уже проходил трижды»). Полное
    // сведение к одному источнику здесь не оправдано: скрипт проверяет ещё DNS,
    // порты, SSL и время ответа, а из REQUIRED в него в принципе годятся только
    // пути с ожиданием 200 - остальные проверяет другой класс запроса (401 без
    // токена). Но там, где пути СОВПАДАЮТ по написанию, коды обязаны совпадать -
    // иначе диагностика с домена будет спорить с дымом деплоя об одном и том же адресе.
    it('пути check_bibliaris.sh, совпадающие с дымом деплоя, ожидают тот же код', () => {
      const lines = checkBibliaris().split(/\r?\n/);
      const start = lines.findIndex((l) => /^\s*endpoints=\(\s*$/.test(l));
      expect(start).toBeGreaterThan(-1);

      // Строки берутся ВСЕ, как написаны, и разбираются общим `parseEntry` -
      // тем же, что разбирает массив дыма выката. Отбор по форме здесь запрещён
      // по той же причине, что описана у `endpointEntries()` выше: молчаливо
      // пропущенная строка (без кавычек, с хвостовым комментарием, в форме
      // «путь код») выпала бы из сверки, и кейс остался бы зелёным, ничего
      // не сверив. Своя копия разбора «кавычки + путь» этого не умела.
      ...
      // Форма записи - голый путь в кавычках, и это не косметика: цикл скрипта
      // подставляет элемент в URL целиком (`https://$DOMAIN$endpoint`), поэтому
      // запись в форме дыма выката («путь ожидаемый_код») собрала бы адрес
      // `https://домен/api/health/readiness 200` и краснела бы на исправном проде.
      // Ожидаемый код у этого списка один на всех и зашит в сравнение - 200.
      for (const entry of entries) {
        expect(entry).toMatch(/^"\/api\/\S*"$/);
      }

      const paths = entries.map((entry) => parseEntry(entry).path);
      const shared = paths.filter((path) => path in REQUIRED);
      // 🔴 Непустое пересечение проверяется отдельно, до сверки кодов. Без этой
      // строки кейс вырожден: `/api/health/readiness` - сегодня единственная общая
      // запись, и стоит ей уехать из скрипта (переименование, `?verbose=1`, другой
      // префикс), цикл ниже не выполнит ни одного `expect`, а кейс останется
      // зелёным - то есть сверка двух списков исчезнет ровно тогда, когда списки
      // и начали расходиться.
      expect(shared.length).toBeGreaterThan(0);

      for (const path of shared) {
        // check_bibliaris.sh считает успехом только "200" (см. `if [[ "$response"
        // == "200" ]]` в каждой из его проверок) - если дым деплоя для того же
        // пути ждёт другой код, списки уже разъехались.
