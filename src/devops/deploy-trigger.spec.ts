...
describe('LEGACY-241: окно выката объявляется, закрывается и не открывается само', () => {
  ...
  describe('границы окна обслуживания', () => {
    ...
    /**
     * 🔴 `LEGACY-364`. С 03.09.2026 e2e идут отдельным job'ом `e2e`, а не хвостом
     * job'а `test`. Отказ e2e оставляет `deploy` в состоянии `skipped`, поэтому
     * без явного упоминания этого job'а отказ уходил бы в канал двумя разными
     * способами неверно: либо уведомление не поднималось бы вовсе (условие шага
     * смотрело только на `test`, `build` и `deploy`), либо называло бы стадией
     * «выкат (skipped)» — и человек шёл бы читать логи выката, которого не было.
     *
     * ⚠️ Проверяется и условие шага, и то, что стадия считается по своей ветке:
     * одного `needs` в job'е мало — он лишь даёт доступ к результату, а решение
     * о тексте принимает `run`.
     */
    it('отказ e2e поднимает уведомление и называется своей стадией', () => {
      const failed = step('notify', FAILED);

      // Условие шага обязано видеть отказ e2e: без этого при `deploy: skipped`
      // уведомление молчит, и выкат встаёт без единого сообщения.
      expect(stepCondition(failed)).toContain("needs.e2e.result == 'failure'");

      const body = failed.lines.join('\n');
      // Результат прокинут через `env`, а не подставлен в тело `run` выражением:
      // то же правило, что и для версии в этом же шаге.
      expect(body).toMatch(/E2E_RESULT:\s*\$\{\{\s*needs\.e2e\.result\s*\}\}/);
      // И читается: переменная в окружении без ветки в `run` ничего не меняет.
      expect(body).toContain('E2E_RESULT');
      expect(body).toMatch(/STAGE="e2e/);
    });
