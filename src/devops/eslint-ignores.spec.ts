 ...
 * а не списывается на лишние минуты. То есть `ignores` здесь — уже не ускорение, а условие
 * зелёного гейта, и снимать эти строки ради покрытия нельзя, не разобравшись с содержимым.
 * Сам `.eslintignore` по-прежнему не читает ни `tsc`, ни `jest`, ни один шаг конвейера.
 */
