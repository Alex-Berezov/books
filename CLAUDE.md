...
6. **Не ставить и не обновлять зависимости по ходу дела.** Любая правка блоков
   ...
   четырёх тем владельца. Позови подагента `arbiter` (`D:/newDev/.claude/agents/arbiter.md`), дай ему
...
8. **Не объявлять работу сделанной при красных проверках.** Слово «готово» допустимо только вместе
   с фактическим выводом `node D:/newDev/.claude/hooks/gates.js`. Пересказ результата вместо вывода
...
| `yarn check:response-schema` | быстрая       | всегда, когда дифф трогает DTO ответа, контроллер или снимок OpenAPI                             |
...
- **Prisma только напрямую.** Приведение клиента к своему интерфейсу (`as unknown as SomeDelegate`,
  ...
  доживает до прода. С 05.09.2026 это сверяет `yarn delegate-check` (`LEGACY-045`): он стоит
  в `scripts/ci.sh` и в `deploy.yml` рядом с `drift-check`, красным отвечает на обращение
  к несуществующей модели - через точку, через `prisma['model']` и через `*DatabaseClient`,
  который объявляет модель не из схемы. Чего он **не** видит: получателя без типа и без единого
  верного вызова в том же файле (нечем подтвердить, что это клиент Prisma), и модель, которую
  читают только связью (`select: { translations: true }`) - такая числится «never accessed»
  в справочном разделе вывода, гейт этим не краснеет.
...
- **`any` ловится линтом с 11.09.2026** (`LEGACY-181`): правило `no-explicit-any`
...
- **Контроллеры исключений не ловят** (`LEGACY-179`, 13.09.2026). `BookController` был
  ...
  Сервис бросает исключение Nest ранним `throw`; голый `Error` наружу превращается в 500
  вместо внятного кода. Сторож на возврат - `src/modules/book/book.controller.errors.spec.ts`.
...
раздел «Ревьюеры по принадлежности диффа») и встроенное code-review, сводит находки
в таблицу, прогоняет `node D:/newDev/.claude/hooks/gates.js` и обнуляет счётчик правок.
Копилка уроков - `D:/newDev/.claude/qa-index.md` (строка на урок, читается перед задачей
целиком); полные записи лежат в `D:/newDev/.claude/qa-lessons.md` на 77 КБ и читаются **только
точечно**: `grep -n "^## L-0NN"` и `Read` с `offset`. Разовое разрешение на защищённый файл -
строка в `D:/newDev/.claude/unlock.txt` с именем репозитория (`books/<путь>`, см. пункт 4
«Жёстких запретов»); строку убрать **после пуша всех затронутых репозиториев**, а не после
первого коммита (`L-028`: гварды считают дифф против `main`, и закоммиченный, но не запушенный
файл из окна сравнения не уходит), а в ответе назвать файл и причину.
