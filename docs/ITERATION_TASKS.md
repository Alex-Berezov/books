...
- Объём:
  - Ввести `X-Admin-Language` или `adminLang` для админских листингов/создания.
  - Pages admin: фильтр/создание по `language`.
  - Books admin: агрегированный статус версий по языкам (эндпоинт/расширение ответа).
- Критерии приёмки: листинги фильтруются; новые Page/BookVersion сохраняются в выбранном языке.
- Замечания: UI вне задачи.
  - Дополнительно: язык не передаётся в теле создания — сервис проставляет из `X-Admin-Language` (единый глобальный контекст).

## 28) Мультисайт i18n: SEO resolve по языку

- Цель: Корректный SEO-бандл с учётом языка.
- Объём: обновить `GET /seo/resolve` для учёта языка пути/контекста; e2e на версии и страницы.
- Критерии приёмки: e2e зелёные, SEO соответствует языку.
