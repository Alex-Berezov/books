...
1. 18 — Черновики/публикация для книг и версий
...
- Объём:
  - Prisma: `BookVersion.status` (draft|published), `publishedAt`; (опционально `Book.status`).
  - Публичные эндпоинты скрывают draft; админские показывают всё; добавить ручки publish/unpublish.
  - e2e: draft не виден анониму; после publish — виден.
- Критерии приёмки: статусы корректно соблюдаются во всех списках/деталях.
