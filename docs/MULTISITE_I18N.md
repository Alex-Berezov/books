...
## 11) Политика URL для i18n и «нейтральных» API (зафиксировано)

Цель: единая договорённость, чтобы в следующих итерациях не терять язык пользователя и не усложнять URL избыточно.

- Публичный контент (обязательно `/:lang` в пути):
  - `/:lang/books/:slug/overview`, `/:lang/pages/:slug`, `/:lang/categories/:slug/books`, `/:lang/tags/:slug/books`, `/:lang/seo/resolve`.
  - Каноника и SEO зависят от `:lang` — это главный источник истины для публичного UI.
  - Sitemap/robots: `GET /sitemap.xml`, `GET /sitemap-:lang.xml`, `GET /robots.txt` (внутри URL содержат префикс `/:lang`).

- Нейтральные API (без `/:lang` в пути):
  - Auth: `/auth/register`, `/auth/login`, `/auth/refresh`, `/auth/logout`.
  - Технические: `/uploads/*`, `/media/*`, `/likes/*`, `/comments/*`, `/views/*`, `/me/*` (bookshelf/progress), `/versions/:id` и т.п.
  - Контейнеры без привязки к языку: `/books`, `/books/slug/:slug`, `/books/:id`, а также публичный список версий `/books/:bookId/versions` (язык выбирается по `language` или `Accept-Language`).
  - Язык для таких ручек при необходимости передаётся заголовком `Accept-Language` (или опционально `?lang=` для отдельных эндпоинтов согласно документации).

- Админ-контекст языка:
  - URL префикс: `/admin/:lang/...` для страниц и версий книг.
  - Заголовок `X-Admin-Language` имеет приоритет над языком пути.
  - При создании сущностей `Page/BookVersion` поле `language` в DTO игнорируется — берётся из эффективного админ-языка.

- Фронтенд-гайдлайны:
  - UI-роуты всегда содержат `/:lang` для публичных страниц и удобной навигации.
  - На нейтральных экранах (например, `/fr/auth/login`) UI сохраняет текущий язык в состоянии/cookie и передаёт его на бэк через `Accept-Language` и параметр `redirect=/fr/...` для возврата после входа/регистрации.
  - Опционально можно посылать `X-UI-Language: fr` на авторизацию/регистрацию — бэк сможет зафиксировать предпочтение пользователя.

- Источник истины и персист:
  - Для публичного контента — `:lang` в пути (приоритет над `?lang` и `Accept-Language`).
  - Для профиля — `User.languagePreference` (заполняется при регистрации или в настройках пользователя; может использоваться как дефолт).
  - Для нейтральных API — заголовок `Accept-Language` используется как подсказка/фильтр.

Эта схема не теряет язык пользователя, даёт корректный SEO и не перегружает API-маршруты там, где язык на результат не влияет.
