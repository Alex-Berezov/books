...
## ⚠️ ВАЖНОЕ ПРАВИЛО ПЕРЕД ДЕПЛОЕМ

> **🚨 ВСЕГДА запускайте тесты перед push на сервер!**
>
> ```bash
> # Обязательная проверка перед любым commit и push:
> yarn test          # Все unit тесты
> yarn test:e2e      # E2E тесты (если применимо)
> yarn lint          # Проверка кода
> yarn typecheck     # Проверка TypeScript
> ```
>
> **Только после успешного прохождения всех тестов можно делать commit и push!**  
> Это правило существует, чтобы избежать поломки production.
