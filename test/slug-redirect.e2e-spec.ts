...
describe('Slug redirects (LEGACY-062) e2e', () => {
   ...
   * `record()` из транзакции — правка на один символ (`tx` → `this.prisma`), и она
   * не роняет ни одну посадку выше: в успешном сценарии результат тот же. Ломается
   * она только на неудачном обновлении — и ломается молча, оставляя редирект со
   * ЖИВОГО адреса на несуществующий. Посетитель и краулер при этом уводятся со
   * страницы, которая на самом деле открывается.
   *
