...
describe('Slug redirects (LEGACY-062) e2e', () => {
   ...
   * запоминается и id (по нему удаляется сама книга), и исходный слаг — ключ,
   * под которым фикстура запомнила id-шники прав (`LEGACY-200`: до 08.09.2026 они
   * выводились из слага литералом, теперь лежат в реестре фикстуры под тем же ключом).
   */
