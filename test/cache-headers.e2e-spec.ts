...
describe('кэш-заголовки живых ответов', () => {
  ...
  describe('публичный кэш сохранён там, где он и был', () => {
     ...
     * **одинаковое** тело.
     *
     * ⚠️ Здесь стоял ещё один кейс — на безъязыком `GET /seo/resolve`, где
     * заголовок до правки определял язык единолично. Маршрут снят 15.09.2026
     * (`LEGACY-387`), и кейс снят вместе с ним: у оставшейся формы языка
     * в пути не бывает лишь у несуществующего адреса.
     *
     * Этот обработчик зависел от заголовка не напрямую, а через фильтр
     * `available`: язык пути отбрасывался, если сущность на нём не издана,
     * и выбор уходил к заголовку (`seo.service.ts:444-451`).
     *
     * Маршрут выбран не любой публичный, а тот, где дефект жил: первая версия
     * кейса ходила на `/en/books/cards`, а тот обработчик заголовок не читал
     * **никогда** — кейс остался бы зелёным и после полного отката правки,
     * то есть не был посадкой вовсе (`L-004`).
     *
    ...
    it('тело /:lang/seo/resolve не зависит от Accept-Language', async () => {
      ...
      const bare = await request(http()).get('/en/seo/resolve?type=catalog&id=catalog').expect(200);
