...
Локальный запуск нужен только если вы:

- Разрабатываете новые features в backend коде
- Запускаете E2E тесты локально
- Отлаживаете специфичные проблемы

Для остальных случаев (frontend разработка, тестирование API) используйте production URL.
...
## 🚨 КРИТИЧЕСКИ ВАЖНОЕ ПРАВИЛО ДЛЯ РАЗРАБОТЧИКОВ

### ⚠️ ВСЕГДА запускайте тесты ПЕРЕД commit и push!

**Перед каждым `git push` обязательно выполните:**
...
# 4. E2E тесты (если применимо к вашим изменениям)
yarn test:e2e
```

**Правило:** Коммитьте и отправляйте на сервер ТОЛЬКО если все проверки прошли успешно!
...
```bash
yarn ci  # Запускает typecheck, lint и все тесты
```
...
- Исключения: `/health`, `/docs`, `/docs-json`. `/metrics` из исключений убран (LEGACY-072) и закрыт ролью `admin`.
- Отказ — **429 с заголовком `Retry-After`** (до 08.08.2026 был 403 без него: робот читал такой ответ как «доступ закрыт», а не «повтори позже»).
- **Кого считать клиентом:** `TRUSTED_PROXY_CIDRS` — список CIDR, которым разрешено объявлять настоящий адрес посетителя через `CF-Connecting-IP`. По умолчанию — диапазоны Cloudflare, зашитые в `src/common/net/client-ip.ts`. Переменная существует для того, чтобы смена диапазонов CF правилась без выката. Заголовок от источника вне списка **игнорируется** — иначе лимит обходится его подстановкой (LEGACY-064, LEGACY-077).
- 🔴 **`INTERNAL_PROXY_CIDRS` — адреса нашего собственного фронта** (на проде `192.64.118.164/32`). Страницы рендерит один контейнер, и вход в аккаунт выполняет он же, поэтому для API весь сайт приходит с одного адреса. От источника из этого списка принимается заголовок `X-Visitor-IP`, которым фронт передаёт настоящий адрес посетителя. **По умолчанию список пуст**: ошибка в этой настройке означает приём `X-Visitor-IP` от постороннего, то есть выданный всем обход лимита. Пустой список просто отключает эту ступень — лимит считается по адресу фронта, как до правки.
...
- **� Чеклист деплоя:** `docs/DEPLOYMENT_CHECKLIST.md` - **главный чеклист** (22 этапа)
